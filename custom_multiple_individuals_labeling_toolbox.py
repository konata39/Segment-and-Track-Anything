...
from custom_widgets import BasePanel, WidgetPanel, BaseFrame
...
class MainFrame(BaseFrame):
    def __init__(self, parent, config, config3d, sourceCam, jump_unlabeled):
        ...
        self.txt = wx.TextCtrl(self.widget_panel, id=wx.ID_ANY, size=(140,-1))
        self.txt.SetValue('0')
        widgetsizer.Add(self.txt, 1, wx.ALL, 15)
        self.txt.Enable(False)

        self.select_frame = wx.Button(self.widget_panel, id=wx.ID_ANY, label="Select frame")
        widgetsizer.Add(self.select_frame, 1, wx.ALL, 15)
        self.select_frame.Bind(wx.EVT_BUTTON, self.selectImage)
        self.select_frame.Enable(False)
    ...
    def selectImage(self, event):
        """
        Moves to select image
        """
        self.individualrdb.SetSelection(0)
        MainFrame.select_individual(self, event)
        #  Checks for the last image and disables the button
        if len(self.index) - self.iter == 1:
            self.next.Enable(False)
            return
        self.prev.Enable(True)
        ...
        self.buttonCounter = {i: [] for i in self.individual_names}
        MainFrame.saveEachImage(self)
        if not self.txt.GetValue().isdigit():
            return
        self.iter = int(self.txt.GetValue())

        if self.iter >= len(self.index):
            self.iter = len(self.index) - 1
        if self.iter < 0:
            self.iter = 0
